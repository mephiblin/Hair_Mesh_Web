...
> 구현 상태: 2026-08-09 기준 primitive 1–7단계, FFD stack 8–12단계, 다중 Control 선택·이동과 3ds Max식 Viewport 조작 13–15단계, Viewport 회귀 고정 16단계, FFD 선택 피드백·편집 토글·대상별 RMB 메뉴 17단계, 잠긴 root의 Viewport 선택 차단 18단계 완료. 현재 동작 계약과 코드 지도는 [`features/proxy-mesh.md`](features/proxy-mesh.md)를 우선한다. 이 문서는 도입 당시 IA와 단계별 의사결정 기록이다.
...
- Viewport regression: Axis Lines OFF에서 긴 guide hidden + 기본 Translate helper visible/enabled, Proxy 표면 drag와 한 단계 Undo, FFD Ctrl 다중 선택의 노란 표시·동시 drag·Edit/Finish 토글, `Sphere002 → Box001` click-through 선택, Proxy/Curve RMB 명령, 잠긴 Curve/Proxy의 LMB·RMB 차단과 Scene Explorer 복구, 1024×768 overflow 0을 `npm run test:viewport`로 자동 확인.
