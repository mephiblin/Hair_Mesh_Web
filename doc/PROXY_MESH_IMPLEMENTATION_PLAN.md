...
> 구현 상태: 2026-08-09 기준 primitive 1–7단계, FFD stack 8–12단계, 다중 Control 선택·이동과 3ds Max식 Viewport 조작 13–15단계, Viewport 회귀 고정 16단계, FFD 선택 피드백·편집 토글·대상별 RMB 메뉴 17단계, 잠긴 root의 Viewport 선택 차단 18단계, Curve/Proxy 무선택 상태 19단계 완료. 현재 동작 계약과 코드 지도는 [`features/proxy-mesh.md`](features/proxy-mesh.md)를 우선한다. 이 문서는 도입 당시 IA와 단계별 의사결정 기록이다.
...
18. Locked root pick policy: 잠긴 Curve/Proxy를 Viewport LMB·RMB·직접 drag·Edit/FFD click-through raycast 후보에서 제외하고, 잠금 해제를 위해 Scene Explorer 선택 경로는 유지.
19. Empty root selection: `orbit`/`transform` 빈 LMB와 단일 root 삭제를 `clearObjectSelection()`으로 통합해 Scene 강조, Control 선택, lattice, gizmo와 Modify 문맥을 함께 비우고 첫 객체 fallback을 제거.
...
- 잠긴 Curve/Proxy는 Viewport LMB·RMB와 직접 drag로 선택되지 않으며 Scene Explorer에서는 선택하고 잠금을 해제할 수 있다.
- Select/Object 모드의 빈 공간 클릭과 단일 Proxy 삭제 뒤에는 첫 Curve/Proxy를 강제 선택하지 않고 `No Selection`/Modify `NONE`/gizmo 해제 상태가 된다.
- Reference가 없어도 보이는 Proxy 표면에 Line Point를 배치하고, Export에는 최종 FFD 결과를 bake한다.

## 완료 검증 기록

- Node `npm run check`: 31/31 계약 통과. 4종 parameter clamp/topology/winding, FFD resolution/identity/Bernstein/stack/order/project round-trip, Control 선택 연산, Window/Crossing 판정, 3ds Max POV direct/V-menu key mapping, ViewCube face/edge/corner·방향/up-vector/drag 회전량, context menu 화면 clamp와 hidden/locked Viewport pick 정책 포함.
...
- Viewport regression: 3ds Max `T/B/F/L/P/U`·`V→K`와 typing 차단, ViewCube face/free-drag/keyboard/Home·Ortho/compact bounds와 선택 배지 분리, 빈 공간 클릭·단일 삭제 뒤 무선택, Axis Lines OFF에서 긴 guide hidden + 기본 Translate helper visible/enabled, Proxy 표면 drag와 한 단계 Undo, FFD Ctrl 다중 선택의 노란 표시·동시 drag·Edit/Finish 토글, `Sphere002 → Box001` click-through 선택, Proxy/Curve RMB 명령, 잠긴 Curve/Proxy의 LMB·RMB 차단과 Scene Explorer 복구, 1024×768 overflow 0을 `npm run test:viewport`로 자동 확인.
