...
| Viewport reference images | `loadReferenceImage()`, `disposeReferenceImage()` | PNG/JPEG/WebP/SVG, session-only plane |
...
- Front/Left/Back Viewport Reference Image는 `modelRoot`와 별개의 표시 plane이다. 이미지 binary는 프로젝트/OBJ/FBX에 포함하지 않고, 프로젝트에는 정렬값과 파일명 힌트만 저장한다.
- Viewport Reference Image의 Texture/Material/Geometry/Object URL은 view별 교체·Clear에서 해제한다.
...
- Front/Left/Back 이미지가 Export 대상 Mesh나 Reference surface raycast 후보에 섞이지 않고, 프로젝트 JSON에도 이미지 payload가 포함되지 않는가?
...
- Browser: 각 Reference/Brush 형식의 성공·실패, 모든 Reference material preset과 Original 복귀, Wire Off/Wire Only/Surface + Wire와 선 색상, Front/Left/Back 이미지 로드·교체·Clear, 여러 Brush 추가/삭제, 프로젝트 round-trip.
