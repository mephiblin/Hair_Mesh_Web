...
| Viewport reference Planes | `loadReferenceImage()`, `disposeReferenceImage()` | PNG/JPEG/WebP/SVG, session-only Plane Mesh |
...
- Front/Left/Back Viewport Reference Image는 `modelRoot`와 별개의 표시 Plane Mesh이다. Perspective에 표시되고 자체 3D transform을 가지지만 이미지 binary나 Plane geometry는 프로젝트/OBJ/FBX에 포함하지 않는다. 프로젝트에는 transform·표시·Flip·Back-face Cull과 파일명 힌트만 저장한다.
- Viewport Reference Plane의 Texture/Material/Geometry/선택 outline/Object URL은 view별 교체·Clear에서 해제한다.
...
- Front/Left/Back Plane이 3D transform 후에도 Export 대상 Mesh나 Reference surface raycast 후보에 섞이지 않고, 프로젝트 JSON에도 이미지 payload가 포함되지 않는가?
